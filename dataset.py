...
def generate_dataset(num_samples=12000):
    data = []
    for _ in range(num_samples):
        ...
        # Simulate match_score with a direct relationship to avg_quality_score
        match_score = 0.0
        ...
        # Adjust match score based on average quality score (direct relationship with match_score)
        match_score += 0.5 * (avg_quality_score)  # Ensure avg_quality_score boosts match score

    
        # Normalize match score to be between 0 and 1
        match_score = min(match_score, 1.0)

        # Ensure that match_score stays in the desired range (0.8 to 1.0)
        if match_score < 0.8:
            match_score = round(np.random.uniform(0.8, 1.0), 2)  # Manually force match scores to be in the desired range

        # Adjust avg_quality_score according to match_score
        if match_score >= 0.8:
            avg_quality_score = round(np.random.uniform(0.8, 1.0), 2)  # Make sure avg_quality_score is high
        else:
            avg_quality_score = round(np.random.uniform(0.0, 0.8), 2)  # Lower avg_quality_score for low match scores
...
# Save to CSV
th_file_path = "backend/data/task_user_pairs.csv"
